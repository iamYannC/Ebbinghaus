...
1. [trials.csv — Phase 1: Stimulus Creation](#trialscsv--phase-1-stimulus-creation)
2. [prompts.csv — Phase 2: Evaluation](#promptscsv--phase-2-evaluation)
3. [evals.csv — Phase 2: Evaluation (legacy)](#evalscsv--phase-2-evaluation-legacy)
...
trials <- generate_design(seed = 42, n_per_tier = 5)  # 5 trials per tier
...
| 5 | `model` | character | Full model ID, e.g., `"gpt-5.4-pro"`, `"claude-sonnet-4-6"` | Model config | Model identifier. Set in `models` list. |
...
models <- list(
  ...
  list(provider = "openai", model = "gpt-5.4-pro", temperature = 0, max_tokens = 512),
  list(provider = "google", model = "gemini-3.1-pro-preview", temperature = 0, max_tokens = 512),
  
  # Test temperature effects
  list(provider = "openai", model = "gpt-5.4-pro", temperature = 0.5, max_tokens = 512),
  list(provider = "openai", model = "gpt-5.4-pro", temperature = 1.0, max_tokens = 512)
)
...
| `model` | character | **Yes** | Provider-specific model ID (e.g., `"gpt-5.4-pro"`, `"claude-sonnet-4-6"`, `"gemini-3.1-pro-preview"`) | Model to use. |
...
models <- list(
  list(provider = "openai", model = "gpt-5.4-pro", temperature = 0, max_tokens = 512),
  list(provider = "anthropic", model = "claude-sonnet-4-6", temperature = 0, max_tokens = 512),
  list(provider = "google", model = "gemini-3.1-pro-preview", temperature = 0, max_tokens = 512)
)
...
models <- list(
  list(provider = "openai", model = "gpt-5.4-pro", temperature = 0, max_tokens = 512),
  list(provider = "openai", model = "gpt-5.4-pro", temperature = 0.3, max_tokens = 512),
  list(provider = "openai", model = "gpt-5.4-pro", temperature = 0.7, max_tokens = 512),
  list(provider = "openai", model = "gpt-5.4-pro", temperature = 1.0, max_tokens = 512)
)
